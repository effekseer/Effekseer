				...
				if (m_cullingWorld != NULL)
				{
					...
					if (effect->Culling.Shape == CullingShape::Sphere)
					{
...

						{
                            ...
                            Vec3f culling_pos = Vec3f::Transform(Vec3f(effect->Culling.Location),
                                                                 pInstance->GetGlobalMatrix43());
                            ds.CullingObjectPointer->SetPosition(Culling3D::Vector3DF(culling_pos.GetX(),
                                                                                      culling_pos.GetY(),
                                                                                      culling_pos.GetZ()));
						}
