...
vec4 _main(PS_Input Input)
{
    ...
    vec3 texNormal = (texture2D(Sampler_g_normalSampler, Input.UV + UVOffset).xyz - vec3(0.5)) * 2.0;
    ...
    vec3 _352 = Output.xyz * ((CBPS0.fLightColor.xyz * diffuse) + CBPS0.fLightAmbient.xyz);
    ...
    vec3 _437 = Output.xyz * CBPS0.fEmissiveScaling.x;
    Output = vec4(_437.x, _437.y, _437.z, Output.w);
    if (Output.w <= max(0.0, advancedParam.AlphaThreshold))
    {
        discard;
    }
    vec3 _468 = mix(CBPS0.fEdgeColor.xyz * CBPS0.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - CBPS0.fEdgeParameter.x)));
