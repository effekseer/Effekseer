...
vec4 _main(PS_Input Input)
{
    ...
    Output = param_7;
    float diffuse = max(dot(CBPS0.fLightDirection.xyz, localNormal), 0.0);
    vec3 _458 = Output.xyz * ((CBPS0.fLightColor.xyz * diffuse) + CBPS0.fLightAmbient.xyz);
    ...
    if (CBPS0.fFalloffParameter.x == 1.0)
    {
        ...
        if (CBPS0.fFalloffParameter.y == 0.0)
        {
            vec3 _504 = Output.xyz + FalloffBlendColor.xyz;
            Output = vec4(_504.x, _504.y, _504.z, Output.w);
        }
        else
        {
            if (CBPS0.fFalloffParameter.y == 1.0)
            {
                vec3 _517 = Output.xyz - FalloffBlendColor.xyz;
                Output = vec4(_517.x, _517.y, _517.z, Output.w);
            }
            else
            {
                if (CBPS0.fFalloffParameter.y == 2.0)
                {
                    vec3 _530 = Output.xyz * FalloffBlendColor.xyz;
                    Output = vec4(_530.x, _530.y, _530.z, Output.w);
                }
            }
        }
        Output.w *= FalloffBlendColor.w;
    }
    vec3 _544 = Output.xyz * CBPS0.fEmissiveScaling.x;
    Output = vec4(_544.x, _544.y, _544.z, Output.w);
    ...
    vec3 _642 = mix(CBPS0.fEdgeColor.xyz * CBPS0.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - CBPS0.fEdgeParameter.x)));
    Output = vec4(_642.x, _642.y, _642.z, Output.w);
    return Output;
}
...
void main()
{
    ...
    vec4 _684 = _main(Input);
    _entryPointOutput = _684;
}
...
highp float SoftParticle(highp float backgroundZ, highp float meshZ, highp vec4 softparticleParam, highp vec4 reconstruct1, highp vec4 reconstruct2)
{
    ...
    highp float alphaFar = abs(depth.y - depth.x) / distanceFar;
    highp float alphaNear = (abs(depth.y) - distanceNearOffset) / distanceNear;
    return min(max(min(alphaFar, alphaNear), 0.0), 1.0);
}
...
highp vec4 _main(PS_Input Input)
{
    ...
    if (CBPS0.fFalloffParameter.x == 1.0)
    {
        ...
        if (CBPS0.fFalloffParameter.y == 0.0)
        {
            highp vec3 _504 = Output.xyz + FalloffBlendColor.xyz;
            Output = vec4(_504.x, _504.y, _504.z, Output.w);
        }
        else
        {
            if (CBPS0.fFalloffParameter.y == 1.0)
            {
                highp vec3 _517 = Output.xyz - FalloffBlendColor.xyz;
                Output = vec4(_517.x, _517.y, _517.z, Output.w);
            }
            else
            {
                if (CBPS0.fFalloffParameter.y == 2.0)
                {
                    highp vec3 _530 = Output.xyz * FalloffBlendColor.xyz;
                    Output = vec4(_530.x, _530.y, _530.z, Output.w);
                }
            }
        }
    ...
    highp vec3 _544 = Output.xyz * CBPS0.fEmissiveScaling.x;
    Output = vec4(_544.x, _544.y, _544.z, Output.w);
    ...
    highp vec3 _642 = mix(CBPS0.fEdgeColor.xyz * CBPS0.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - CBPS0.fEdgeParameter.x)));
    Output = vec4(_642.x, _642.y, _642.z, Output.w);
    return Output;
}
