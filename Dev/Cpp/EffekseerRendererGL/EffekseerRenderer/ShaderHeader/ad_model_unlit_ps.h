...
vec4 _main(PS_Input Input)
{
    ...
    vec3 _481 = Output.xyz * CBPS0.fEmissiveScaling.x;
    ...
    vec3 _579 = mix(CBPS0.fEdgeColor.xyz * CBPS0.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - CBPS0.fEdgeParameter.x)));
    Output = vec4(_579.x, _579.y, _579.z, Output.w);
    return Output;
}
...
void main()
{
    ...
    vec4 _615 = _main(Input);
    _entryPointOutput = _615;
}
...
highp vec4 _main(PS_Input Input)
{
    ...
    if (CBPS0.fFalloffParameter.x == 1.0)
    {
        ...
        if (CBPS0.fFalloffParameter.y == 0.0)
        {
            highp vec3 _441 = Output.xyz + FalloffBlendColor.xyz;
            Output = vec4(_441.x, _441.y, _441.z, Output.w);
        }
        else
        {
            if (CBPS0.fFalloffParameter.y == 1.0)
            {
                highp vec3 _454 = Output.xyz - FalloffBlendColor.xyz;
                Output = vec4(_454.x, _454.y, _454.z, Output.w);
            }
            else
            {
                if (CBPS0.fFalloffParameter.y == 2.0)
                {
                    highp vec3 _467 = Output.xyz * FalloffBlendColor.xyz;
                    Output = vec4(_467.x, _467.y, _467.z, Output.w);
                }
            }
        }
    ...
    highp vec3 _481 = Output.xyz * CBPS0.fEmissiveScaling.x;
    Output = vec4(_481.x, _481.y, _481.z, Output.w);
    ...
    highp vec3 _579 = mix(CBPS0.fEdgeColor.xyz * CBPS0.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - CBPS0.fEdgeParameter.x)));
    Output = vec4(_579.x, _579.y, _579.z, Output.w);
    return Output;
}
