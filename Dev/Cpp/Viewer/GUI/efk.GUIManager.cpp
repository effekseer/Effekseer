...
namespace efk
{
void ResizeBicubic(uint32_t* dst, int32_t dstWidth, int32_t dstHeight, int32_t dstStride, const uint32_t* src, int32_t srcWidth, int32_t srcHeight, int32_t srcStride)
{
	...
	auto weight = [](float d) -> float {
		const float a = -1.0f;
		return d <= 1.0f ? ((a + 2.0f) * d * d * d) - ((a + 3.0f) * d * d) + 1 : d <= 2.0f ? (a * d * d * d) - (5.0f * a * d * d) + (8.0f * a * d) - (4.0f * a) : 0.0f;
	};
...
bool DragFloatN(const char* label, float* v, int components, float v_speed, float* v_min, float* v_max, const char* display_format1, const char* display_format2, const char* display_format3, float power)
{
...
bool DragIntN(const char* label, int* v, int components, int v_speed, int* v_min, int* v_max, const char* display_format1, const char* display_format2, const char* display_format3)
{
	ImGuiWindow* window = ImGui::GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool GUIManager::DragFloat1EfkEx(const char16_t* label, float* v, float v_speed, float v_min, float v_max, const char16_t* display_format1, float power)
{
	...
	return DragFloatN(
		utf8str<256>(label), v, 1, v_speed, v_min_, v_max_, utf8str<256>(display_format1), nullptr, nullptr, power);
}
...
bool GUIManager::DragFloat3EfkEx(const char16_t* label, float* v, float v_speed, float v_min1, float v_max1, float v_min2, float v_max2, float v_min3, float v_max3, const char16_t* display_format1, const char16_t* display_format2, const char16_t* display_format3, float power)
{
	...
	return DragFloatN(
		utf8str<256>(label), v, 3, v_speed, v_min_, v_max_, utf8str<256>(display_format1), utf8str<256>(display_format2), utf8str<256>(display_format3), power);
}
