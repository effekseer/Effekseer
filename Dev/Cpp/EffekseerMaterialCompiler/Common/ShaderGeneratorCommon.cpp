...
std::string GetFixedGradient(const char* name, const Gradient& gradient)
{
	...
	for (int32_t i = 0; i < gradient.Colors.size(); i++)
	{
		...
		ss << "g.colors[" << i << "].x = " << key.Color[0] * key.Intensity << ";" << std::endl;
		ss << "g.colors[" << i << "].y = " << key.Color[1] * key.Intensity << ";" << std::endl;
		ss << "g.colors[" << i << "].z = " << key.Color[2] * key.Intensity << ";" << std::endl;
		ss << "g.colors[" << i << "].w = " << key.Position << ";" << std::endl;
	}

	for (int32_t i = 0; i < gradient.Alphas.size(); i++)
	{
		const auto key = getAlphaKey(gradient, i);
		ss << "g.alphas[" << i << "].x = " << key.Alpha << ";" << std::endl;
		ss << "g.alphas[" << i << "].y = " << key.Position << ";" << std::endl;
	}
