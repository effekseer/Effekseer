...
void* ModelLoader::Load(const void* data, int32_t size)
{
	...
	for (int32_t f = 0; f < model->GetFrameCount(); f++)
	{
		...
		hr =
			device->CreateVertexBuffer(sizeof(Effekseer::Model::VertexWithIndex) * model->InternalModels[f].VertexCount * model->ModelCount,
									   D3DUSAGE_WRITEONLY,
									   0,
									   D3DPOOL_MANAGED,
									   &vb,
		...
		if (FAILED(hr))
		{
			...
			hr = device->CreateVertexBuffer(sizeof(Effekseer::Model::VertexWithIndex) * model->InternalModels[f].VertexCount *
												model->ModelCount,
											D3DUSAGE_WRITEONLY,
											0,
											D3DPOOL_DEFAULT,
		...
		hr = device->CreateIndexBuffer(sizeof(Effekseer::Model::Face) * model->InternalModels[f].FaceCount * model->ModelCount,
									   D3DUSAGE_WRITEONLY,
									   D3DFMT_INDEX32,
									   D3DPOOL_MANAGED,
									   &ib,
