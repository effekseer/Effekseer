...
{
    float4 _187_g_scale : register(c0);
    float4 _187_mUVInversedBack : register(c1);
    float4 _187_fFlipbookParameter : register(c2);
    float4 _187_fUVDistortionParameter : register(c3);
    float4 _187_fBlendTextureParameter : register(c4);
};
...
float4 _main(PS_Input Input)
{
    ...
    float4 Output = tex2D(Sampler_g_sampler, Input.UV + UVOffset);
    Output.w *= Input.Color.w;
    float4 param_1 = Output;
    float param_2 = advancedParam.FlipbookRate;
    ApplyFlipbook(param_1, _187_fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_2, Sampler_g_sampler);
    Output = param_1;
