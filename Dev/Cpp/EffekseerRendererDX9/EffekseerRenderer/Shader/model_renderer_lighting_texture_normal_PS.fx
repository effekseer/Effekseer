...
float4 _main(PS_Input Input)
{
    ...
    float3 texNormal = (tex2D(Sampler_g_normalSampler, Input.UV + UVOffset).xyz - 0.5f.xxx) * 2.0f;
    ...
    Output = param_3;
    float diffuse = max(dot(_187_fLightDirection.xyz, localNormal), 0.0f);
    float3 _352 = Output.xyz * ((_187_fLightColor.xyz * diffuse) + _187_fLightAmbient.xyz);
    ...
    float3 _437 = Output.xyz * _187_fEmissiveScaling.x;
    Output = float4(_437.x, _437.y, _437.z, Output.w);
    if (Output.w <= max(0.0f, advancedParam.AlphaThreshold))
    {
        discard;
    }
    float3 _468 = lerp(_187_fEdgeColor.xyz * _187_fEdgeParameter.y, Output.xyz, ceil((Output.w - advancedParam.AlphaThreshold) - _187_fEdgeParameter.x).xxx);
