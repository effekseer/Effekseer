...
{
    float4 _393_g_scale : register(c0);
    float4 _393_mUVInversedBack : register(c1);
    float4 _393_fFlipbookParameter : register(c2);
    float4 _393_fUVDistortionParameter : register(c3);
    float4 _393_fBlendTextureParameter : register(c4);
    float4 _393_softParticleParam : register(c5);
    float4 _393_reconstructionParam1 : register(c6);
    float4 _393_reconstructionParam2 : register(c7);
};
...
float4 _main(PS_Input Input)
{
    ...
    bool param_6 = false;
    ApplyFlipbook(param_4, _393_fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_5, param_6, Sampler_sampler_colorTex);
    Output = param_4;
