...
layout(set = 1, binding = 2) uniform sampler2D Sampler_sampler_uvDistortionTex;
layout(set = 1, binding = 0) uniform sampler2D Sampler_sampler_colorTex;
layout(set = 1, binding = 1) uniform sampler2D Sampler_sampler_alphaTex;
layout(set = 1, binding = 5) uniform sampler2D Sampler_sampler_blendUVDistortionTex;
layout(set = 1, binding = 3) uniform sampler2D Sampler_sampler_blendTex;
layout(set = 1, binding = 4) uniform sampler2D Sampler_sampler_blendAlphaTex;
layout(set = 1, binding = 6) uniform sampler2D Sampler_sampler_depthTex;
...
vec4 _main(PS_Input Input)
{
    ...
    vec2 param_2 = _433.fUVDistortionParameter.zw;
    ...
    ApplyFlipbook(param_6, _433.fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_7, param_8, Sampler_sampler_colorTex);
    Output = param_6;
    ...
    vec2 param_12 = _433.fUVDistortionParameter.zw;
    ...
    {
        vec3 cameraVec = normalize(-_433.fCameraFrontDirection.xyz);
        float CdotN = clamp(dot(cameraVec, normalize(Input.WorldN)), 0.0, 1.0);
        vec4 FalloffBlendColor = mix(_433.fFalloffEndColor, _433.fFalloffBeginColor, vec4(pow(CdotN, _433.fFalloffParameter.z)));
    ...
    vec4 _677 = Output;
    vec3 _679 = _677.xyz * _433.fEmissiveScaling.x;
    Output.x = _679.x;
    Output.y = _679.y;
    Output.z = _679.z;
    ...
    if (_433.softParticleParam.w != 0.0)
    {
        ...
        vec4 param_21 = _433.softParticleParam;
        vec4 param_22 = _433.reconstructionParam1;
        vec4 param_23 = _433.reconstructionParam2;
    ...
    vec4 _765 = Output;
    float _768 = Output.w;
    vec3 _777 = mix(_433.fEdgeColor.xyz * _433.fEdgeParameter.y, _765.xyz, vec3(ceil((_768 - advancedParam.AlphaThreshold) - _433.fEdgeParameter.x)));
    Output.x = _777.x;
    Output.y = _777.y;
    Output.z = _777.z;
