...
layout(location = 2) in vec3 Input_WorldP;
layout(location = 3) in vec3 Input_WorldN;
layout(location = 4) in vec3 Input_WorldT;
layout(location = 5) in vec3 Input_WorldB;
...
vec4 _main(PS_Input Input)
{
    ...
    diffuse = max(dot(_187.fLightDirection.xyz, localNormal), 0.0);
    vec4 Output = texture(Sampler_g_colorSampler, Input.UV + UVOffset) * Input.VColor;
    vec4 param_1 = Output;
    float param_2 = advancedParam.FlipbookRate;
    ApplyFlipbook(param_1, _187.fFlipbookParameter, Input.VColor, advancedParam.FlipbookNextIndexUV + UVOffset, param_2, Sampler_g_colorSampler);
    Output = param_1;
    ...
    Output = param_3;
    vec3 _347 = Output.xyz * _187.fEmissiveScaling.x;
    Output = vec4(_347.x, _347.y, _347.z, Output.w);
    if (Output.w <= max(0.0, advancedParam.AlphaThreshold))
    {
        discard;
    }
    vec3 _372 = Output.xyz * (vec3(diffuse, diffuse, diffuse) + vec3(_187.fLightAmbient.xyz));
    Output = vec4(_372.x, _372.y, _372.z, Output.w);
    vec3 _393 = mix(_187.fEdgeColor.xyz * _187.fEdgeParameter.y, Output.xyz, vec3(ceil((Output.w - advancedParam.AlphaThreshold) - _187.fEdgeParameter.x)));
