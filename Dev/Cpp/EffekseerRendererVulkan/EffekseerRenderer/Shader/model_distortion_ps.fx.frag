...
vec4 _main(PS_Input Input)
{
    ...
    float xscale = (((Output.x * 2.0) - 1.0) * Input.Color.x) * _150.g_scale.x;
    float yscale = (((Output.y * 2.0) - 1.0) * Input.Color.y) * _150.g_scale.x;
    vec2 uv = (pos + ((posR - pos) * xscale)) + ((posU - pos) * yscale);
    uv.x = (uv.x + 1.0) * 0.5;
    uv.y = 1.0 - ((uv.y + 1.0) * 0.5);
    uv.y = _150.mUVInversedBack.x + (_150.mUVInversedBack.y * uv.y);
    vec3 color = vec3(texture(Sampler_sampler_backTex, uv).xyz);
    Output = vec4(color.x, color.y, color.z, Output.w);
    vec4 screenPos = Input.PosP / vec4(Input.PosP.w);
    vec2 screenUV = (screenPos.xy + vec2(1.0)) / vec2(2.0);
    screenUV.y = 1.0 - screenUV.y;
    if (!(_150.softParticleParam.w == 0.0))
    {
        float backgroundZ = texture(Sampler_sampler_depthTex, screenUV).x;
        float param = backgroundZ;
        float param_1 = screenPos.z;
        vec4 param_2 = _150.softParticleParam;
        vec4 param_3 = _150.reconstructionParam1;
        vec4 param_4 = _150.reconstructionParam2;
