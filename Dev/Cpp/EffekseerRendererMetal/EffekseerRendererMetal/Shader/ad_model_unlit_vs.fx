...
static inline __attribute__((always_inline))
void CalculateAndStoreAdvancedParameter(thread const float2& uv, thread const float2& uv1, thread const float4& alphaUV, thread const float4& uvDistortionUV, thread const float4& blendUV, thread const float4& blendAlphaUV, thread const float4& blendUVDistortionUV, thread const float& flipbookIndexAndNextRate, thread const float& modelAlphaThreshold, thread VS_Output& vsoutput, constant VS_ConstantBuffer& _412)
{
    ...
    float4 param_2 = _412.flipbookParameter1;
    float4 param_3 = _412.flipbookParameter2;
    float param_4 = flipbookIndexAndNextRate;
    float2 param_5 = uv1;
    float2 param_6 = float2(_412.mUVInversed.xy);
...
static inline __attribute__((always_inline))
VS_Output _main(VS_Input Input, constant VS_ConstantBuffer& _412)
{
    uint index = Input.Index;
    float4x4 mModel = transpose(_412.mModel_Inst[index]);
    float4 uv = _412.fUV[index];
    float4 alphaUV = _412.fAlphaUV[index];
    float4 uvDistortionUV = _412.fUVDistortionUV[index];
    float4 blendUV = _412.fBlendUV[index];
    float4 blendAlphaUV = _412.fBlendAlphaUV[index];
    float4 blendUVDistortionUV = _412.fBlendUVDistortionUV[index];
    float4 modelColor = _412.fModelColor[index] * Input.Color;
    ...
    Output.PosVS = _412.mCameraProj * worldPos;
    ...
    float4 worldNormal = fast::normalize(float4(localNormal, 0.0) * mModel);
    ...
    CalculateAndStoreAdvancedParameter(param_1, param_2, param_3, param_4, param_5, param_6, param_7, param_8, param_9, param_10, _412);
    Output = param_10;
    Output.PosP = Output.PosVS;
    return Output;
}

vertex main0_out main0(main0_in in [[stage_in]], constant VS_ConstantBuffer& _412 [[buffer(0)]], uint gl_InstanceIndex [[instance_id]])
{
    main0_out out = {};
    VS_Input Input;
    ...
    VS_Output flattenTemp = _main(Input, _412);
