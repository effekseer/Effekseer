...
VS_Output _main(VS_Input Input, constant VS_ConstantBuffer& v_27)
{
    ...
    float3 _112 = normalize(Input.Normal * lightMat);
    localNormal = float4(_112.x, _112.y, _112.z, localNormal.w);
    float4 localBinormal = float4(0.0, 0.0, 0.0, 1.0);
    float3 _119 = normalize(Input.Binormal * lightMat);
    localBinormal = float4(_119.x, _119.y, _119.z, localBinormal.w);
    float4 localTangent = float4(0.0, 0.0, 0.0, 1.0);
    float3 _126 = normalize(Input.Tangent * lightMat);
    localTangent = float4(_126.x, _126.y, _126.z, localTangent.w);
