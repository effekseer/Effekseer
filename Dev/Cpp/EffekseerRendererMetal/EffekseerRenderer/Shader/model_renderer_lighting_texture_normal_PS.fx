...
float4 _main(PS_Input Input, thread texture2d<float> g_uvDistortionTexture, thread sampler g_uvDistortionSampler, constant PS_ConstanBuffer& v_187, thread texture2d<float> g_colorTexture, thread sampler g_colorSampler, thread texture2d<float> g_normalTexture, thread sampler g_normalSampler, thread texture2d<float> g_alphaTexture, thread sampler g_alphaSampler, thread texture2d<float> g_blendUVDistortionTexture, thread sampler g_blendUVDistortionSampler, thread texture2d<float> g_blendTexture, thread sampler g_blendSampler, thread texture2d<float> g_blendAlphaTexture, thread sampler g_blendAlphaSampler)
{
    ...
    ApplyFlipbook(param_1, g_colorTexture, g_colorSampler, v_187.fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_2);
    Output = param_1;
    ...
    Output = param_3;
    float diffuse = fast::max(dot(v_187.fLightDirection.xyz, localNormal), 0.0);
    float3 _352 = Output.xyz * ((v_187.fLightColor.xyz * diffuse) + v_187.fLightAmbient.xyz);
    Output = float4(_352.x, _352.y, _352.z, Output.w);
    if (v_187.fFalloffParam.Param.x == 1.0)
    {
        float3 cameraVec = normalize(-v_187.fCameraFrontDirection.xyz);
        float CdotN = fast::clamp(dot(cameraVec, float3(localNormal.x, localNormal.y, localNormal.z)), 0.0, 1.0);
        float4 FalloffBlendColor = mix(v_187.fFalloffParam.EndColor, v_187.fFalloffParam.BeginColor, float4(pow(CdotN, v_187.fFalloffParam.Param.z)));
        if (v_187.fFalloffParam.Param.y == 0.0)
        {
            float3 _398 = Output.xyz + FalloffBlendColor.xyz;
            Output = float4(_398.x, _398.y, _398.z, Output.w);
        }
        else
        {
            if (v_187.fFalloffParam.Param.y == 1.0)
            {
                float3 _411 = Output.xyz - FalloffBlendColor.xyz;
                Output = float4(_411.x, _411.y, _411.z, Output.w);
            }
            else
            {
                if (v_187.fFalloffParam.Param.y == 2.0)
                {
                    float3 _424 = Output.xyz * FalloffBlendColor.xyz;
...
fragment main0_out main0(main0_in in [[stage_in]], constant PS_ConstanBuffer& v_187 [[buffer(0)]], texture2d<float> g_uvDistortionTexture [[texture(0)]], texture2d<float> g_colorTexture [[texture(1)]], texture2d<float> g_normalTexture [[texture(2)]], texture2d<float> g_alphaTexture [[texture(3)]], texture2d<float> g_blendUVDistortionTexture [[texture(4)]], texture2d<float> g_blendTexture [[texture(5)]], texture2d<float> g_blendAlphaTexture [[texture(6)]], sampler g_uvDistortionSampler [[sampler(0)]], sampler g_colorSampler [[sampler(1)]], sampler g_normalSampler [[sampler(2)]], sampler g_alphaSampler [[sampler(3)]], sampler g_blendUVDistortionSampler [[sampler(4)]], sampler g_blendSampler [[sampler(5)]], sampler g_blendAlphaSampler [[sampler(6)]], float4 gl_FragCoord [[position]])
{
    ...
    float4 _511 = _main(Input, g_uvDistortionTexture, g_uvDistortionSampler, v_187, g_colorTexture, g_colorSampler, g_normalTexture, g_normalSampler, g_alphaTexture, g_alphaSampler, g_blendUVDistortionTexture, g_blendUVDistortionSampler, g_blendTexture, g_blendSampler, g_blendAlphaTexture, g_blendAlphaSampler);
    out._entryPointOutput = _511;
    return out;
}
