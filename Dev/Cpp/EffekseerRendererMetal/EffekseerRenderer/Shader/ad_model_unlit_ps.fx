...
static inline __attribute__((always_inline))
float4 ConvertToScreen(thread const float4& c, constant PS_ConstanBuffer& v_365)
{
    if (v_365.miscFlags.x == 0.0)
    {
        return c;
    }
    float4 param = c;
...
static inline __attribute__((always_inline))
float4 _main(PS_Input Input, constant PS_ConstanBuffer& v_365, thread texture2d<float> _uvDistortionTex, thread sampler sampler_uvDistortionTex, thread texture2d<float> _colorTex, thread sampler sampler_colorTex, thread texture2d<float> _alphaTex, thread sampler sampler_alphaTex, thread texture2d<float> _blendUVDistortionTex, thread sampler sampler_blendUVDistortionTex, thread texture2d<float> _blendTex, thread sampler sampler_blendTex, thread texture2d<float> _blendAlphaTex, thread sampler sampler_blendAlphaTex, thread texture2d<float> _depthTex, thread sampler sampler_depthTex)
{
    PS_Input param = Input;
    AdvancedParameter advancedParam = DisolveAdvancedParameter(param);
    float2 param_1 = advancedParam.UVDistortionUV;
    float2 param_2 = v_365.fUVDistortionParameter.zw;
    float2 UVOffset = UVDistortionOffset(_uvDistortionTex, sampler_uvDistortionTex, param_1, param_2);
    UVOffset *= v_365.fUVDistortionParameter.x;
    ...
    ApplyFlipbook(param_4, _colorTex, sampler_colorTex, v_365.fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_5);
    Output = param_4;
    ...
    ApplyTextureBlending(param_9, BlendTextureColor, v_365.fBlendTextureParameter.x);
    Output = param_9;
    if (v_365.fFalloffParameter.x == 1.0)
    {
        float3 cameraVec = normalize(-v_365.fCameraFrontDirection.xyz);
        float CdotN = fast::clamp(dot(cameraVec, normalize(Input.WorldN)), 0.0, 1.0);
        float4 FalloffBlendColor = mix(v_365.fFalloffEndColor, v_365.fFalloffBeginColor, float4(pow(CdotN, v_365.fFalloffParameter.z)));
        if (v_365.fFalloffParameter.y == 0.0)
        {
        ...
        {
            if (v_365.fFalloffParameter.y == 1.0)
            {
            ...
            {
                if (v_365.fFalloffParameter.y == 2.0)
                {
    ...
    float3 _590 = Output.xyz * v_365.fEmissiveScaling.x;
    ...
    screenUV.y = v_365.mUVInversedBack.x + (v_365.mUVInversedBack.y * screenUV.y);
    if ((isunordered(v_365.softParticleParam.w, 0.0) || v_365.softParticleParam.w != 0.0))
    {
        float backgroundZ = _depthTex.sample(sampler_depthTex, screenUV).x;
        float param_10 = backgroundZ;
        float param_11 = screenPos.z;
        float4 param_12 = v_365.softParticleParam;
        float4 param_13 = v_365.reconstructionParam1;
        float4 param_14 = v_365.reconstructionParam2;
...
fragment main0_out main0(main0_in in [[stage_in]], constant PS_ConstanBuffer& v_365 [[buffer(0)]], texture2d<float> _uvDistortionTex [[texture(2)]], texture2d<float> _colorTex [[texture(0)]], texture2d<float> _alphaTex [[texture(1)]], texture2d<float> _blendUVDistortionTex [[texture(5)]], texture2d<float> _blendTex [[texture(3)]], texture2d<float> _blendAlphaTex [[texture(4)]], texture2d<float> _depthTex [[texture(6)]], sampler sampler_uvDistortionTex [[sampler(2)]], sampler sampler_colorTex [[sampler(0)]], sampler sampler_alphaTex [[sampler(1)]], sampler sampler_blendUVDistortionTex [[sampler(5)]], sampler sampler_blendTex [[sampler(3)]], sampler sampler_blendAlphaTex [[sampler(4)]], sampler sampler_depthTex [[sampler(6)]], float4 gl_FragCoord [[position]])
{
    ...
    float4 _722 = _main(Input, v_365, _uvDistortionTex, sampler_uvDistortionTex, _colorTex, sampler_colorTex, _alphaTex, sampler_alphaTex, _blendUVDistortionTex, sampler_blendUVDistortionTex, _blendTex, sampler_blendTex, _blendAlphaTex, sampler_blendAlphaTex, _depthTex, sampler_depthTex);
    out._entryPointOutput = _722;
    return out;
}
