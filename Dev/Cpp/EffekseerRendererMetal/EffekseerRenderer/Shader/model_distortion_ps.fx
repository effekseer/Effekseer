...
static inline __attribute__((always_inline))
float4 _main(PS_Input Input, thread texture2d<float> _colorTex, thread sampler sampler_colorTex, constant PS_ConstanBuffer& v_150, thread texture2d<float> _backTex, thread sampler sampler_backTex, thread texture2d<float> _depthTex, thread sampler sampler_depthTex)
{
    ...
    float xscale = (((Output.x * 2.0) - 1.0) * Input.Color.x) * v_150.g_scale.x;
    float yscale = (((Output.y * 2.0) - 1.0) * Input.Color.y) * v_150.g_scale.x;
    float2 uv = (pos + ((posR - pos) * xscale)) + ((posU - pos) * yscale);
    uv.x = (uv.x + 1.0) * 0.5;
    uv.y = 1.0 - ((uv.y + 1.0) * 0.5);
    uv.y = v_150.mUVInversedBack.x + (v_150.mUVInversedBack.y * uv.y);
    ...
    if ((isunordered(v_150.softParticleParam.w, 0.0) || v_150.softParticleParam.w != 0.0))
    {
        float backgroundZ = _depthTex.sample(sampler_depthTex, screenUV).x;
        float param = backgroundZ;
        float param_1 = screenPos.z;
        float4 param_2 = v_150.softParticleParam;
        float4 param_3 = v_150.reconstructionParam1;
        float4 param_4 = v_150.reconstructionParam2;
...
fragment main0_out main0(main0_in in [[stage_in]], constant PS_ConstanBuffer& v_150 [[buffer(0)]], texture2d<float> _colorTex [[texture(0)]], texture2d<float> _backTex [[texture(1)]], texture2d<float> _depthTex [[texture(2)]], sampler sampler_colorTex [[sampler(0)]], sampler sampler_backTex [[sampler(1)]], sampler sampler_depthTex [[sampler(2)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    PS_Input Input;
    ...
    float4 _305 = _main(Input, _colorTex, sampler_colorTex, v_150, _backTex, sampler_backTex, _depthTex, sampler_depthTex);
    out._entryPointOutput = _305;
    return out;
}
