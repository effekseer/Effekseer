...
float4 _main(PS_Input Input, thread texture2d<float> g_texture, thread sampler g_sampler, constant _Global& v_73, thread texture2d<float> g_backTexture, thread sampler g_backSampler)
{
...
fragment main0_out main0(main0_in in [[stage_in]], constant _Global& v_73 [[buffer(0)]], texture2d<float> g_texture [[texture(0)]], texture2d<float> g_backTexture [[texture(1)]], sampler g_sampler [[sampler(0)]], sampler g_backSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    PS_Input Input;
