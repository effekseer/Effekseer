...
VS_Output _main(VS_Input Input, constant VS_ConstantBuffer& v_27)
{
    ...
    Output.Pos = v_27.mCameraProj * localPosition;
    Output.Color = modelColor;
    Output.UV.x = (Input.UV.x * uv.z) + uv.x;
    Output.UV.y = (Input.UV.y * uv.w) + uv.y;
    float3x3 lightMat = float3x3(matModel[0].xyz, matModel[1].xyz, matModel[2].xyz);
    float4 localNormal = float4(0.0, 0.0, 0.0, 1.0);
    float3 _111 = normalize(Input.Normal * lightMat);
    localNormal = float4(_111.x, _111.y, _111.z, localNormal.w);
    float4 localBinormal = float4(0.0, 0.0, 0.0, 1.0);
    float3 _118 = normalize(Input.Binormal * lightMat);
    localBinormal = float4(_118.x, _118.y, _118.z, localBinormal.w);
    float4 localTangent = float4(0.0, 0.0, 0.0, 1.0);
    float3 _125 = normalize(Input.Tangent * lightMat);
    localTangent = float4(_125.x, _125.y, _125.z, localTangent.w);
