...
static inline __attribute__((always_inline))
float4 ConvertFromSRGBTexture(thread const float4& c, constant PS_ConstanBuffer& v_195)
{
    if (v_195.miscFlags.x == 0.0)
    {
        return c;
    }
    float4 param = c;
...
static inline __attribute__((always_inline))
float4 ConvertToScreen(thread const float4& c, constant PS_ConstanBuffer& v_195)
{
    if (v_195.miscFlags.x == 0.0)
    {
        return c;
...
static inline __attribute__((always_inline))
float4 _main(PS_Input Input, constant PS_ConstanBuffer& v_195, thread texture2d<float> _colorTex, thread sampler sampler_colorTex, thread texture2d<float> _normalTex, thread sampler sampler_normalTex, thread texture2d<float> _depthTex, thread sampler sampler_depthTex)
{
    float4 param = _colorTex.sample(sampler_colorTex, Input.UV);
    float4 Output = ConvertFromSRGBTexture(param, v_195) * Input.Color;
    ...
    float diffuse = fast::max(dot(v_195.fLightDirection.xyz, localNormal), 0.0);
    float3 _301 = Output.xyz * ((v_195.fLightColor.xyz * diffuse) + v_195.fLightAmbient.xyz);
    Output = float4(_301.x, _301.y, _301.z, Output.w);
    float3 _309 = Output.xyz * v_195.fEmissiveScaling.x;
    ...
    screenUV.y = v_195.mUVInversedBack.x + (v_195.mUVInversedBack.y * screenUV.y);
    if ((isunordered(v_195.softParticleParam.w, 0.0) || v_195.softParticleParam.w != 0.0))
    {
        float backgroundZ = _depthTex.sample(sampler_depthTex, screenUV).x;
        float param_1 = backgroundZ;
        float param_2 = screenPos.z;
        float4 param_3 = v_195.softParticleParam;
        float4 param_4 = v_195.reconstructionParam1;
        float4 param_5 = v_195.reconstructionParam2;
    ...
    return ConvertToScreen(param_6, v_195);
}

fragment main0_out main0(main0_in in [[stage_in]], constant PS_ConstanBuffer& v_195 [[buffer(0)]], texture2d<float> _colorTex [[texture(0)]], texture2d<float> _normalTex [[texture(1)]], texture2d<float> _depthTex [[texture(2)]], sampler sampler_colorTex [[sampler(0)]], sampler sampler_normalTex [[sampler(1)]], sampler sampler_depthTex [[sampler(2)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    PS_Input Input;
    ...
    float4 _415 = _main(Input, v_195, _colorTex, sampler_colorTex, _normalTex, sampler_normalTex, _depthTex, sampler_depthTex);
