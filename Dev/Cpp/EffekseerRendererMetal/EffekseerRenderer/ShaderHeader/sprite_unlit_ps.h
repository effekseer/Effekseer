...
static inline __attribute__((always_inline))
float4 _main(PS_Input Input, thread texture2d<float> g_texture, thread sampler g_sampler, thread texture2d<float> g_depthTexture, thread sampler g_depthSampler, constant PS_ConstanBuffer& v_124)
{
...
fragment main0_out main0(main0_in in [[stage_in]], constant PS_ConstanBuffer& v_124 [[buffer(0)]], texture2d<float> g_texture [[texture(0)]], texture2d<float> g_depthTexture [[texture(1)]], sampler g_sampler [[sampler(0)]], sampler g_depthSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    main0_out out = {};
    PS_Input Input;
    ...
    float4 _184 = _main(Input, g_texture, g_sampler, g_depthTexture, g_depthSampler, v_124);
