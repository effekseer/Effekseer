...
static inline __attribute__((always_inline))
float4 _main(PS_Input Input, thread texture2d<float> _uvDistortionTex, thread sampler sampler_uvDistortionTex, constant PS_ConstanBuffer& v_283, thread texture2d<float> _colorTex, thread sampler sampler_colorTex, thread texture2d<float> _normalTex, thread sampler sampler_normalTex, thread texture2d<float> _alphaTex, thread sampler sampler_alphaTex, thread texture2d<float> _blendUVDistortionTex, thread sampler sampler_blendUVDistortionTex, thread texture2d<float> _blendTex, thread sampler sampler_blendTex, thread texture2d<float> _blendAlphaTex, thread sampler sampler_blendAlphaTex, thread texture2d<float> _depthTex, thread sampler sampler_depthTex)
{
    PS_Input param = Input;
    AdvancedParameter advancedParam = DisolveAdvancedParameter(param);
    float2 param_1 = advancedParam.UVDistortionUV;
    float2 param_2 = v_283.fUVDistortionParameter.zw;
    float2 UVOffset = UVDistortionOffset(_uvDistortionTex, sampler_uvDistortionTex, param_1, param_2);
    UVOffset *= v_283.fUVDistortionParameter.x;
    ...
    ApplyFlipbook(param_3, _colorTex, sampler_colorTex, v_283.fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_4);
    Output = param_3;
    ...
    ApplyTextureBlending(param_7, BlendTextureColor, v_283.fBlendTextureParameter.x);
    Output = param_7;
    float diffuse = fast::max(dot(v_283.fLightDirection.xyz, localNormal), 0.0);
    float3 _458 = Output.xyz * ((v_283.fLightColor.xyz * diffuse) + v_283.fLightAmbient.xyz);
    Output = float4(_458.x, _458.y, _458.z, Output.w);
    if (v_283.fFalloffParameter.x == 1.0)
    {
        float3 cameraVec = normalize(-v_283.fCameraFrontDirection.xyz);
        float CdotN = fast::clamp(dot(cameraVec, float3(localNormal.x, localNormal.y, localNormal.z)), 0.0, 1.0);
        float4 FalloffBlendColor = mix(v_283.fFalloffEndColor, v_283.fFalloffBeginColor, float4(pow(CdotN, v_283.fFalloffParameter.z)));
        if (v_283.fFalloffParameter.y == 0.0)
        {
            float3 _504 = Output.xyz + FalloffBlendColor.xyz;
            Output = float4(_504.x, _504.y, _504.z, Output.w);
        }
        else
        {
            if (v_283.fFalloffParameter.y == 1.0)
            {
                float3 _517 = Output.xyz - FalloffBlendColor.xyz;
                Output = float4(_517.x, _517.y, _517.z, Output.w);
            }
            else
            {
                if (v_283.fFalloffParameter.y == 2.0)
                {
                    float3 _530 = Output.xyz * FalloffBlendColor.xyz;
                    Output = float4(_530.x, _530.y, _530.z, Output.w);
                }
            }
        }
        Output.w *= FalloffBlendColor.w;
    }
    float3 _544 = Output.xyz * v_283.fEmissiveScaling.x;
    Output = float4(_544.x, _544.y, _544.z, Output.w);
    float4 screenPos = Input.PosP / float4(Input.PosP.w);
    float2 screenUV = (screenPos.xy + float2(1.0)) / float2(2.0);
    screenUV.y = 1.0 - screenUV.y;
    screenUV.y = v_283.mUVInversedBack.x + (v_283.mUVInversedBack.y * screenUV.y);
    if ((isunordered(v_283.softParticleParam.w, 0.0) || v_283.softParticleParam.w != 0.0))
    {
        float backgroundZ = _depthTex.sample(sampler_depthTex, screenUV).x;
        float param_8 = backgroundZ;
        float param_9 = screenPos.z;
        float4 param_10 = v_283.softParticleParam;
        float4 param_11 = v_283.reconstructionParam1;
        float4 param_12 = v_283.reconstructionParam2;
...
fragment main0_out main0(main0_in in [[stage_in]], constant PS_ConstanBuffer& v_283 [[buffer(0)]], texture2d<float> _uvDistortionTex [[texture(3)]], texture2d<float> _colorTex [[texture(0)]], texture2d<float> _normalTex [[texture(1)]], texture2d<float> _alphaTex [[texture(2)]], texture2d<float> _blendUVDistortionTex [[texture(6)]], texture2d<float> _blendTex [[texture(4)]], texture2d<float> _blendAlphaTex [[texture(5)]], texture2d<float> _depthTex [[texture(7)]], sampler sampler_uvDistortionTex [[sampler(3)]], sampler sampler_colorTex [[sampler(0)]], sampler sampler_normalTex [[sampler(1)]], sampler sampler_alphaTex [[sampler(2)]], sampler sampler_blendUVDistortionTex [[sampler(6)]], sampler sampler_blendTex [[sampler(4)]], sampler sampler_blendAlphaTex [[sampler(5)]], sampler sampler_depthTex [[sampler(7)]], float4 gl_FragCoord [[position]])
{
    ...
    float4 _680 = _main(Input, _uvDistortionTex, sampler_uvDistortionTex, v_283, _colorTex, sampler_colorTex, _normalTex, sampler_normalTex, _alphaTex, sampler_alphaTex, _blendUVDistortionTex, sampler_blendUVDistortionTex, _blendTex, sampler_blendTex, _blendAlphaTex, sampler_blendAlphaTex, _depthTex, sampler_depthTex);
    out._entryPointOutput = _680;
    return out;
}
