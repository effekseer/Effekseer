...
static inline __attribute__((always_inline))
float4 _main(PS_Input Input, thread texture2d<float> _uvDistortionTex, thread sampler sampler_uvDistortionTex, constant PS_ConstanBuffer& v_281, thread texture2d<float> _colorTex, thread sampler sampler_colorTex, thread texture2d<float> _alphaTex, thread sampler sampler_alphaTex, thread texture2d<float> _blendUVDistortionTex, thread sampler sampler_blendUVDistortionTex, thread texture2d<float> _blendTex, thread sampler sampler_blendTex, thread texture2d<float> _blendAlphaTex, thread sampler sampler_blendAlphaTex, thread texture2d<float> _depthTex, thread sampler sampler_depthTex)
{
    PS_Input param = Input;
    AdvancedParameter advancedParam = DisolveAdvancedParameter(param);
    float2 param_1 = advancedParam.UVDistortionUV;
    float2 param_2 = v_281.fUVDistortionParameter.zw;
    float2 UVOffset = UVDistortionOffset(_uvDistortionTex, sampler_uvDistortionTex, param_1, param_2);
    UVOffset *= v_281.fUVDistortionParameter.x;
    float4 Output = _colorTex.sample(sampler_colorTex, (Input.UV_Others.xy + UVOffset)) * Input.Color;
    float4 param_3 = Output;
    float param_4 = advancedParam.FlipbookRate;
    ApplyFlipbook(param_3, _colorTex, sampler_colorTex, v_281.fFlipbookParameter, Input.Color, advancedParam.FlipbookNextIndexUV + UVOffset, param_4);
    Output = param_3;
    ...
    ApplyTextureBlending(param_7, BlendTextureColor, v_281.fBlendTextureParameter.x);
    Output = param_7;
    if (v_281.fFalloffParameter.x == 1.0)
    {
        float3 cameraVec = normalize(-v_281.fCameraFrontDirection.xyz);
        float CdotN = fast::clamp(dot(cameraVec, normalize(Input.WorldN)), 0.0, 1.0);
        float4 FalloffBlendColor = mix(v_281.fFalloffEndColor, v_281.fFalloffBeginColor, float4(pow(CdotN, v_281.fFalloffParameter.z)));
        if (v_281.fFalloffParameter.y == 0.0)
        {
        ...
        {
            if (v_281.fFalloffParameter.y == 1.0)
            {
            ...
            {
                if (v_281.fFalloffParameter.y == 2.0)
                {
    ...
    float3 _480 = Output.xyz * v_281.fEmissiveScaling.x;
    ...
    if ((isunordered(v_281.softParticleParam.w, 0.0) || v_281.softParticleParam.w != 0.0))
    {
        ...
        float4 param_10 = v_281.softParticleParam;
        float4 param_11 = v_281.reconstructionParam1;
        float4 param_12 = v_281.reconstructionParam2;
