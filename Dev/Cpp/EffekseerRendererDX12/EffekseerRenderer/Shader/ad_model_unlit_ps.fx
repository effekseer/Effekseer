...
cbuffer PS_ConstanBuffer : register(b1)
{
    ...
    float4 _282_fFalloffParameter : packoffset(c7);
    float4 _282_fFalloffBeginColor : packoffset(c8);
    float4 _282_fFalloffEndColor : packoffset(c9);
...
float4 _main(PS_Input Input)
{
    ...
    Output = param_7;
    if (_282_fFalloffParameter.x == 1.0f)
    {
        ...
        float4 FalloffBlendColor = lerp(_282_fFalloffEndColor, _282_fFalloffBeginColor, pow(CdotN, _282_fFalloffParameter.z).xxxx);
        if (_282_fFalloffParameter.y == 0.0f)
        {
            float3 _441 = Output.xyz + FalloffBlendColor.xyz;
            Output = float4(_441.x, _441.y, _441.z, Output.w);
        }
        else
        {
            if (_282_fFalloffParameter.y == 1.0f)
            {
                float3 _454 = Output.xyz - FalloffBlendColor.xyz;
                Output = float4(_454.x, _454.y, _454.z, Output.w);
            }
            else
            {
                if (_282_fFalloffParameter.y == 2.0f)
                {
                    float3 _467 = Output.xyz * FalloffBlendColor.xyz;
                    Output = float4(_467.x, _467.y, _467.z, Output.w);
                }
            }
        }
        Output.w *= FalloffBlendColor.w;
    }
    float3 _481 = Output.xyz * _282_fEmissiveScaling.x;
    Output = float4(_481.x, _481.y, _481.z, Output.w);
    ...
    float3 _575 = lerp(_282_fEdgeColor.xyz * _282_fEdgeParameter.y, Output.xyz, ceil((Output.w - advancedParam.AlphaThreshold) - _282_fEdgeParameter.x).xxx);
