...
{
    float4 _150_g_scale : packoffset(c0);
    float4 _150_mUVInversedBack : packoffset(c1);
    float4 _150_fFlipbookParameter : packoffset(c2);
    float4 _150_fUVDistortionParameter : packoffset(c3);
    float4 _150_fBlendTextureParameter : packoffset(c4);
    float4 _150_softParticleParam : packoffset(c5);
    float4 _150_reconstructionParam1 : packoffset(c6);
    float4 _150_reconstructionParam2 : packoffset(c7);
};
...
float4 _main(PS_Input Input)
{
    ...
    float xscale = (((Output.x * 2.0f) - 1.0f) * Input.Color.x) * _150_g_scale.x;
    float yscale = (((Output.y * 2.0f) - 1.0f) * Input.Color.y) * _150_g_scale.x;
    float2 uv = (pos + ((posR - pos) * xscale)) + ((posU - pos) * yscale);
    uv.x = (uv.x + 1.0f) * 0.5f;
    uv.y = 1.0f - ((uv.y + 1.0f) * 0.5f);
    uv.y = _150_mUVInversedBack.x + (_150_mUVInversedBack.y * uv.y);
    ...
    if (_150_softParticleParam.w != 0.0f)
    {
        float backgroundZ = _depthTex.Sample(sampler_depthTex, screenUV).x;
        float param = backgroundZ;
        float param_1 = screenPos.z;
        float4 param_2 = _150_softParticleParam;
        float4 param_3 = _150_reconstructionParam1;
        float4 param_4 = _150_reconstructionParam2;
...
void frag_main()
{
    ...
    float4 _305 = _main(Input);
    _entryPointOutput = _305;
}
