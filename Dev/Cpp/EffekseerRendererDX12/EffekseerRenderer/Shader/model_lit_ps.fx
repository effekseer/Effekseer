...
{
    float4 _159_fLightDirection : packoffset(c0);
    float4 _159_fLightColor : packoffset(c1);
    float4 _159_fLightAmbient : packoffset(c2);
    float4 _159_fFlipbookParameter : packoffset(c3);
    float4 _159_fUVDistortionParameter : packoffset(c4);
    float4 _159_fBlendTextureParameter : packoffset(c5);
    float4 _159_fCameraFrontDirection : packoffset(c6);
    float4 _159_fFalloffParameter : packoffset(c7);
    float4 _159_fFalloffBeginColor : packoffset(c8);
    float4 _159_fFalloffEndColor : packoffset(c9);
    float4 _159_fEmissiveScaling : packoffset(c10);
    float4 _159_fEdgeColor : packoffset(c11);
    float4 _159_fEdgeParameter : packoffset(c12);
    float4 _159_softParticleParam : packoffset(c13);
    float4 _159_reconstructionParam1 : packoffset(c14);
    float4 _159_reconstructionParam2 : packoffset(c15);
    float4 _159_mUVInversedBack : packoffset(c16);
};
...
float4 _main(PS_Input Input)
{
    ...
    float diffuse = max(dot(_159_fLightDirection.xyz, localNormal), 0.0f);
    float3 _179 = Output.xyz * ((_159_fLightColor.xyz * diffuse) + _159_fLightAmbient.xyz);
    Output = float4(_179.x, _179.y, _179.z, Output.w);
    float3 _188 = Output.xyz * _159_fEmissiveScaling.x;
    ...
    screenUV.y = _159_mUVInversedBack.x + (_159_mUVInversedBack.y * screenUV.y);
    if (_159_softParticleParam.w != 0.0f)
    {
        float backgroundZ = _depthTex.Sample(sampler_depthTex, screenUV).x;
        float param = backgroundZ;
        float param_1 = screenPos.z;
        float4 param_2 = _159_softParticleParam;
        float4 param_3 = _159_reconstructionParam1;
        float4 param_4 = _159_reconstructionParam2;
...
void frag_main()
{
    ...
    float4 _293 = _main(Input);
    _entryPointOutput = _293;
}
