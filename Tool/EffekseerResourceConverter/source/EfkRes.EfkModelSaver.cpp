...
bool EfkModelSaver::Save(std::string_view filepath, const Model& model)
{
	...
	for (auto& node : model.nodes)
	{
		...
		else if (version >= 6)
		{
			...
			{
				auto position = QuantizeFloat32(node.transform.TransformPosition(vertex.position * modelScale));
				auto normal = node.transform.TransformDirection(vertex.normal);
				auto binormal = node.transform.TransformDirection(vertex.binormal);
				auto tangent = node.transform.TransformDirection(vertex.tangent);

				double sign = copysign(1.0, DotProduct(binormal, CrossProduct(normal, tangent)));
				auto octNormal = QuantizeUnorm16(OctNormalEncode(normal));
				auto octTangent = QuantizeUnorm16(OctTangentEncode(tangent, sign));
				...
				writer.Write(position);
				writer.Write(octNormal);
				writer.Write(octTangent);
				writer.Write(uv1);
				writer.Write(uv2);
				writer.Write(color);
			}
