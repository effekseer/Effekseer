...
- `python -m pip install cmake-format` で `cmake-format` を導入してください。
- `python Script/Format_CMake.py` を実行して `CMakeLists.txt` と `*.cmake` を整形します（プラットフォーム共通）。
- スクリプトは生成物やサードパーティコードの再整形を避けるため、`.git`、`build`、`Dev/Cpp/3rdParty` を除外します。
